...
int main() {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        auto directions = {Direction::FORWARD, Direction::BACKWARD, Direction::LEFT, Direction::RIGHT};
        for (auto direction1 : directions) {
            for (auto direction2 : directions) {
                for (auto direction3 : directions) {
                    for (auto direction4 : directions) {
                        for (auto direction5 : directions) {
                            Grid copy = grid;
                            int score = 0;

                            copy.apply(direction1);
                            score += copy.score(valueSum);

                            copy.apply(direction2);
                            score += copy.score(valueSum) * 2;

                            copy.apply(direction3);
                            score += copy.score(valueSum) * 3;

                            copy.apply(direction4);
                            score += copy.score(valueSum) * 4;

                            copy.apply(direction5);
                            score += copy.score(valueSum) * 5;
